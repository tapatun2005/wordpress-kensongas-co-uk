...
class OneEngine{
	...
	public function theme_init() {
		...
		$s_labels = array(
			'name'               => _x( 'Sections', 'post type general name', 'oneengine' ),
			'singular_name'      => _x( 'Section', 'post type singular name', 'oneengine' ),
			'menu_name'          => _x( 'Sections', 'admin menu', 'oneengine' ),
			'name_admin_bar'     => _x( 'Section', 'add new on admin bar', 'oneengine' ),
			'add_new'            => _x( 'Add New', 'Section', 'oneengine' ),
			'add_new_item'       => __( 'Add New Section', 'oneengine' ),
			'new_item'           => __( 'New Section', 'oneengine' ),
			'edit_item'          => __( 'Edit Section', 'oneengine' ),
			'view_item'          => __( 'View Section', 'oneengine' ),
			'all_items'          => __( 'All Sections', 'oneengine' ),
			'search_items'       => __( 'Search Sections', 'oneengine' ),
			'parent_item_colon'  => __( 'Parent Sections:', 'oneengine' ),
			'not_found'          => __( 'No Sections found.', 'oneengine' ),
			'not_found_in_trash' => __( 'No Sections found in Trash.', 'oneengine' ),
		);
		...
		register_post_type( 'section', $s_args );
	...
	public function add_meta_boxes(){
		...
		add_meta_box(
			'section_metabox',
			__( 'Section Data', 'oneengine' ),
			array($this,'section_meta_box_callback'),
			'section',
			'normal',
			'high'
		);		
	...
	public function section_meta_box_callback(){
		global $post;
		wp_enqueue_style( 'wp-color-picker');
		wp_enqueue_script( 'wp-color-picker');		
		wp_nonce_field( 'oe_post_nonce', 'oe_post_nonce' );
		?>
		<label class="team-lbl" for="oe_section_bg"><?php _e('Section Background Color:', 'oneengine') ?></label>
		<input type="text" class="input-custom-field" name="oe_section_bg" id="oe_section_bg" value="<?php echo get_post_meta($post->ID, 'oe_section_bg', true) ?>"><br>

		<label class="team-lbl" for="oe_section_color"><?php _e('Section Text Colour:', 'oneengine') ?></label>
		<input type="text" class="input-custom-field" name="oe_section_color" id="oe_section_color" value="<?php echo get_post_meta($post->ID, 'oe_section_color', true) ?>"><br>

		<label class="team-lbl" for="oe_section_id"><?php _e('Section ID:', 'oneengine') ?></label>
		<input type="text" class="input-custom-field" name="oe_section_id" id="oe_section_id" value="<?php echo get_post_meta($post->ID, 'oe_section_id', true) ?>"><br>

		<script type="text/javascript">
			jQuery(document).ready(function($){
			   $('#oe_section_bg').wpColorPicker();
			   $('#oe_section_color').wpColorPicker();
			});
		</script>
		<style type="text/css">
			label.team-lbl {
	...
	public function save_meta_boxes($post_id){
		...
		if ( get_post_type( $post_id ) ==  'section' ){
	        ...
	        if(isset($_POST['oe_section_bg']))
				update_post_meta( $post_id, 'oe_section_bg', $_POST['oe_section_bg'] );
			
			if(isset($_POST['oe_section_color']))
				update_post_meta( $post_id, 'oe_section_color', $_POST['oe_section_color'] );

			if(isset($_POST['oe_section_id']))
				update_post_meta( $post_id, 'oe_section_id', $_POST['oe_section_id'] );
